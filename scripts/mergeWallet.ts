...
function getUser(input: string): Prisma.UserWhereInput {
  return stringUtils.isUUID(input) ? { id: input } : utils.isAddress(input) ? { wallets: {some: {
    address: input
  }} } : { path: input }
}
...
async function mergeWallet({duplicateAddressOrUserIdOrPath, originalAddressOrUserIdOrPath}: Props) {
  ...
  if (typeof duplicateAddressOrUserIdOrPath !== 'string' || typeof originalAddressOrUserIdOrPath !== 'string' || duplicateAddressOrUserIdOrPath === originalAddressOrUserIdOrPath) {
  ...
  const confirmation = await confirmMerge({duplicateAddressOrUserIdOrPath: duplicateAccount.id, originalAddressOrUserIdOrPath: originalAccount.id})
  ...
  await fs.writeFile(`${__dirname}/merge-wallet-${originalAccount.id}-${Date.now()}.json`, JSON.stringify({
    originalAccount,
    duplicateAccount
  }, null, 2));
