...
async function reimportTemplateSettings({spaceDomain, templatePath, targetSpaceDomain}: {spaceDomain: string; templatePath: string; targetSpaceDomain: string}) {
  ...
  const {oldNewRecordIdHashMap} = await importRoles({exportData: exportedRoles, targetSpaceIdOrDomain: targetSpace.id})
  ...
  const newTemplate = await createProposal({
    authors: [],
    evaluations: proposal.evaluations.map((evaluation) => ({id: uuid(), index: evaluation.index, title: evaluation.title, rubricCriteria: evaluation.rubricCriteria as any, type: evaluation.type, voteSettings: evaluation.voteSettings as any, reviewers: evaluation.reviewers.map((reviewer) => ({userId: targetSpace.createdBy, roleId: reviewer.roleId ? oldNewRecordIdHashMap[reviewer.roleId] : undefined, systemRole: reviewer.systemRole}))})),
    spaceId: targetSpace.id,
    workflowId: existingWorkflow.id,
    formFields: [{id: uuid(), description: '', fieldConfig: {}, index: 0, name: 'title', type: 'short_text', required: true, private: false}],
