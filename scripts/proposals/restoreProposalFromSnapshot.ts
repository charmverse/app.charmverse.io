import { PageType, ProposalAppealReviewer, ProposalEvaluationPermission, ProposalStatus, prisma } from '@charmverse/core/prisma-client';

import { v4 as uuid } from 'uuid';


import { generatePagePathFromPathAndTitle } from '@root/lib/pages';
...
async function restoreProposalFromSnapshot() {
  ...
  const created = await prisma.$transaction(async(tx) => {
    ...
    const newEvaluations = await Promise.all(evaluations.map((evaluation) => {
