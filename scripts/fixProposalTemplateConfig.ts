...
async function getConfig({
  ...
  workflowName: string}) {
    ...
    return {
      template, workflow, similarTemplate
    }
}
...
async function fixEmptyProposalTemplateConfig({
  ...
  spaceDomain: string,
  workflowName: string}): Promise<void> {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await Promise.all(similarTemplate.evaluations.map((evaluation) => tx.proposalEvaluation.create({
        data: {
          ...
          rubricCriteria: {
            createMany: {
              data: evaluation.rubricCriteria.map((criterion) => ({
