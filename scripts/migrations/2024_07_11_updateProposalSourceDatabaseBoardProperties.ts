...
import { RubricEvaluationProperty, SelectedProposalProperties } from 'components/common/DatabaseEditor/components/viewSidebar/viewSourceOptions/components/ProposalSourceProperties/ProposalSourcePropertiesDialog';
  ...
  for (const proposalSourceBoard of proposalSourceBoards) {
    try {
      ...
      const boardSelectedProperties: SelectedProposalProperties = {
        ...
        defaults: cardProperties.filter(cardProperty => defaultProposalPropertyTypes.includes(cardProperty.type)).map(property => property.type),
      ...
      formFields.forEach(formField => {
        const proposalTemplateId = formField.form.proposal.find(proposal => proposal.page?.type === 'proposal_template')?.page?.id;
      ...
      const uniqueRubricTitles = Array.from(
        new Set(
          cardProperties.filter(cardProperty => !!cardProperty.evaluationTitle).map(property => property.evaluationTitle).filter(isTruthy)
        )
      )
      ...
      cardProperties.forEach(property => {
        if (
          ...
          property.type === 'proposalRubricCriteriaReviewerComment' ) && property.evaluationTitle
        ) {
          ...
          proposalTemplateIds.forEach(proposalTemplateId => {
            const proposalTemplateProperty = boardSelectedProperties.templateProperties.find(templateProperty => templateProperty.templateId === proposalTemplateId);
            const rubricEvaluation = rubricEvaluations.find(evaluation => evaluation.title === evaluationTitle && evaluation.proposal.page?.id === proposalTemplateId);
            if (rubricEvaluation) {
                  ...
                  rubricEvaluations: [
                    {
                      ...
                      properties: [
                        ProposalPropertyRubricPropertyRecord[property.type]
                      ],
