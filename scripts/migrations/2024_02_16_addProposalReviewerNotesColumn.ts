...
async function addProposalAuthorColumn() {
  ...
  for (const proposalBoardBlock of proposalBoardBlocks) {
    try {
      let proposalReviewerNotesProperty = (proposalBoardBlock.fields as unknown as BoardFields).cardProperties?.find(boardCardProperty => {
        return boardCardProperty.type === "proposalReviewerNotes"
      }) as IPropertyTemplate

      const transactions: any[] = []

      if (!proposalReviewerNotesProperty) {
        proposalReviewerNotesProperty = proposalDbProperties.proposalReviewerNotes()
      ...
      await prisma.$transaction([
        ...
        ...viewBlocks.map(viewBlock => {
          ...
          const viewBlockVisiblePropertyIds = viewBlockFields.visiblePropertyIds.filter(
            visiblePropertyId => visiblePropertyId !== proposalReviewerNotesProperty.id
            ) || []

          viewBlockVisiblePropertyIds.splice(hasTitleProperty ? 1 : 0, 0, proposalReviewerNotesProperty.id)
