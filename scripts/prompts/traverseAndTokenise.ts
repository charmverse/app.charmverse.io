...
async function traverseAndTokenize(node: FileNode, visited = new Set<FileNode>(), ref = {tokensRemaining: 0}): Promise<{results: string[], tokensUsed: number}> {
...
rl.question('Enter the path to your file: ', async (file) => {
    runDependencyCheck(file, async (data) => {
        ...
        const rootNodes = Array.from(nodes.values()).filter(node => !Array.from(nodes.values()).some(n => n.children.includes(node)));
