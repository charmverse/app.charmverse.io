...
export function useMemberPropertyValues(memberId: string) {
  const { user } = useUser();
  const { data: memberPropertyValues, mutate } = useSWR(`members/${memberId}/values`, () =>
    charmClient.members.getPropertyValues(memberId)
  );

  const getValuesForSpace = useCallback(
    (spaceId: string) => {
  ...
  const canEditSpaceProfile = useCallback(
    (spaceId: string) => {
      if (!user) {
        return false;
      }
  ...
  const updateSpaceValues = useCallback(
    async (spaceId: string, values: UpdateMemberPropertyValuePayload[]) => {
