...
export function usePageDetails(pageIdOrPath: string, spaceId?: string) {
  ...
  } = useSWR(pageIdOrPath ? `pages/details/${pageIdOrPath}` : null, () =>
    charmClient.pages.getPageDetails(pageIdOrPath, spaceId)
  );
  const { mutatePagesList } = usePages();

  const mutatePageDetails = useCallback(
    (updates: PageDetailsUpdates, revalidate = false) => {
      mutateDetails(
        (pageDetailsData) => {
  ...
  const updatePageDetails = useCallback(
    async (updates: PageDetailsUpdates) => {
      ...
      mutatePagesList(
        (pages) => {
          ...
          if (
            currentPageData &&
            (currentPageData.hasContent !== hasContent || currentPageData.galleryImage !== galleryImage)
          ) {
