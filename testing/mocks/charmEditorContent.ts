export const contentWithBlocksAndMarks = {
  type: 'doc',
  content: [
    {
      type: 'paragraph',
      attrs: { track: [] },
      content: [
        {
          text: 'Just a paragraph',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'heading',
      attrs: { id: null, level: 1, track: [], collapseContent: null },
      content: [
        {
          text: 'Heading 1',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'heading',
      attrs: { id: null, level: 2, track: [], collapseContent: null },
      content: [
        {
          text: 'Heading 2',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'heading',
      attrs: { id: null, level: 3, track: [], collapseContent: null },
      content: [
        {
          text: 'Heading 3',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    { type: 'paragraph', attrs: { track: [] } },
    {
      type: 'paragraph',
      attrs: { track: [] },
      content: [
        {
          text: 'Text mark examples ',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: 'bold ',
          type: 'text',
          marks: [
            { type: 'bold' },
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: 'italic ',
          type: 'text',
          marks: [
            { type: 'italic' },
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: 'underline',
          type: 'text',
          marks: [
            { type: 'underline' },
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: ' ',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: 'strike',
          type: 'text',
          marks: [
            { type: 'strike' },
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'paragraph',
      attrs: { track: [] },
      content: [
        {
          type: 'emoji',
          attrs: { emoji: '😀' },
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: ' ',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          type: 'emoji',
          attrs: { emoji: '🚀' },
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: ' ',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          type: 'emoji',
          attrs: { emoji: '🥦' },
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: ' ',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          type: 'emoji',
          attrs: { emoji: '💪' },
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        },
        {
          text: ' ',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:30:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'paragraph',
      attrs: { track: [] },
      content: [
        {
          text: 'Link to charmverse',
          type: 'text',
          marks: [
            { type: 'link', attrs: { href: 'https://app.charmverse.io/' } },
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    { type: 'horizontalRule', attrs: { track: [] } },
    {
      type: 'paragraph',
      attrs: { track: [] },
      content: [
        {
          text: 'Bulleted list',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            },
            { type: 'text-color', attrs: { color: null, bgColor: null } }
          ]
        }
      ]
    },
    {
      type: 'bulletList',
      attrs: { tight: false },
      content: [
        {
          type: 'listItem',
          attrs: { track: [], todoChecked: null },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'some item',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:20:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'listItem',
          attrs: { track: [], todoChecked: null },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'another item',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:20:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'listItem',
          attrs: { track: [], todoChecked: null },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'and yet another item',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:20:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    { type: 'horizontalRule', attrs: { track: [] } },
    {
      type: 'paragraph',
      attrs: { track: [] },
      content: [
        {
          text: 'Ordered list',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T09:20:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'orderedList',
      attrs: { order: 1, tight: false, track: [] },
      content: [
        {
          type: 'listItem',
          attrs: { track: [], todoChecked: null },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'first',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:20:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'listItem',
          attrs: { track: [], todoChecked: null },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'second',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:20:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'listItem',
          attrs: { track: [], todoChecked: null },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'third',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:20:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    { type: 'horizontalRule', attrs: { track: [] } },
    {
      type: 'disclosureDetails',
      content: [
        {
          type: 'disclosureSummary',
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Toggle component',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:30:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'paragraph',
          attrs: { track: [] },
          content: [
            {
              text: 'toggle 1',
              type: 'text',
              marks: [
                {
                  type: 'insertion',
                  attrs: {
                    date: '2023-08-04T09:30:00.000Z',
                    user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                    approved: true,
                    username: 'm.sadura@outlook.com'
                  }
                }
              ]
            }
          ]
        },
        {
          type: 'paragraph',
          attrs: { track: [] },
          content: [
            {
              text: 'toggle 2',
              type: 'text',
              marks: [
                {
                  type: 'insertion',
                  attrs: {
                    date: '2023-08-04T09:30:00.000Z',
                    user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                    approved: true,
                    username: 'm.sadura@outlook.com'
                  }
                }
              ]
            }
          ]
        },
        {
          type: 'paragraph',
          attrs: { track: [] },
          content: [
            {
              text: 'toggle 3',
              type: 'text',
              marks: [
                {
                  type: 'insertion',
                  attrs: {
                    date: '2023-08-04T09:30:00.000Z',
                    user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                    approved: true,
                    username: 'm.sadura@outlook.com'
                  }
                }
              ]
            }
          ]
        },
        { type: 'paragraph', attrs: { track: [] } }
      ]
    },
    {
      type: 'quote',
      attrs: { track: [] },
      content: [
        {
          type: 'paragraph',
          attrs: { track: [] },
          content: [
            {
              text: 'Remarkable quote',
              type: 'text',
              marks: [
                {
                  type: 'insertion',
                  attrs: {
                    date: '2023-08-04T09:30:00.000Z',
                    user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                    approved: true,
                    username: 'm.sadura@outlook.com'
                  }
                }
              ]
            }
          ]
        },
        {
          type: 'paragraph',
          attrs: { track: [] },
          content: [
            {
              text: 'on 2 lines',
              type: 'text',
              marks: [
                {
                  type: 'insertion',
                  attrs: {
                    date: '2023-08-04T09:30:00.000Z',
                    user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                    approved: true,
                    username: 'm.sadura@outlook.com'
                  }
                }
              ]
            }
          ]
        }
      ]
    },
    { type: 'paragraph', attrs: { track: [] } },
    {
      type: 'blockquote',
      attrs: { emoji: '😃', track: [] },
      content: [
        {
          type: 'paragraph',
          attrs: { track: [] },
          content: [
            {
              text: 'Callout content',
              type: 'text',
              marks: [
                {
                  type: 'insertion',
                  attrs: {
                    date: '2023-08-04T09:30:00.000Z',
                    user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                    approved: true,
                    username: 'm.sadura@outlook.com'
                  }
                }
              ]
            }
          ]
        }
      ]
    }
  ]
};

export const contentWithColumnsAndTables = {
  type: 'doc',
  content: [
    {
      type: 'columnLayout',
      attrs: { track: [] },
      content: [
        {
          type: 'columnBlock',
          attrs: { size: 170, track: [] },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Column 1',
                  type: 'text',
                  marks: [
                    { type: 'bold' },
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                },
                {
                  type: 'hardBreak',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                },
                {
                  text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec scelerisque semper sagittis. Quisque at sapien magna. Aenean lobortis, diam vitae posuere efficitur, lacus ante venenatis ex, malesuada viverra nisl massa id felis. Integer at fringilla tortor, accumsan placerat felis. Ut consectetur egestas est, sed auctor nisl dignissim nec.',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'columnBlock',
          attrs: { track: [] },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Column 2',
                  type: 'text',
                  marks: [
                    { type: 'bold' },
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            },
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Etiam purus dui, vestibulum sed euismod quis, pretium ac turpis. Pellentesque sit amet egestas quam, ac commodo leo. In auctor malesuada arcu vitae mattis. Cras sit amet metus finibus nulla varius vestibulum. Etiam tincidunt eros sem, eget varius tortor placerat eu. In venenatis interdum metus a rhoncus. Pellentesque ut libero dapibus, efficitur tortor id, rhoncus justo.',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'columnBlock',
          attrs: { track: [] },
          content: [
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Column 3',
                  type: 'text',
                  marks: [
                    { type: 'bold' },
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            },
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Narrow column',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T09:50:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      type: 'columnLayout',
      attrs: { track: [] },
      content: [
        {
          type: 'columnBlock',
          attrs: { size: 374, track: [] },
          content: [
            {
              type: 'heading',
              attrs: { id: null, level: 2, track: [], collapseContent: null },
              content: [
                {
                  text: 'Column 1',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T10:00:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            },
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Aenean rutrum, ligula non faucibus euismod, ipsum lacus aliquet odio, non tempus tortor neque eu leo. Sed sagittis tellus eget arcu porttitor pretium. Praesent semper tempus eros, vel consectetur eros imperdiet sit amet. Nullam vel mi vel erat pellentesque dignissim. Proin luctus magna augue, sed accumsan sapien gravida eu',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T10:00:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'columnBlock',
          attrs: { track: [] },
          content: [
            {
              type: 'heading',
              attrs: { id: null, level: 2, track: [], collapseContent: null },
              content: [
                {
                  text: 'Column 2',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T10:00:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    }
                  ]
                }
              ]
            },
            {
              type: 'paragraph',
              attrs: { track: [] },
              content: [
                {
                  text: 'Sed vitae lectus non diam finibus pharetra. Donec consequat enim vitae mi aliquet, in vestibulum elit egestas. Donec nec dignissim enim. Proin suscipit libero nec quam viverra condimentum. Mauris quis ex sed purus iaculis molestie id ac diam.',
                  type: 'text',
                  marks: [
                    {
                      type: 'insertion',
                      attrs: {
                        date: '2023-08-04T10:00:00.000Z',
                        user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                        approved: true,
                        username: 'm.sadura@outlook.com'
                      }
                    },
                    { type: 'text-color', attrs: { color: null, bgColor: null } }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      type: 'heading',
      attrs: { id: null, level: 3, track: [], collapseContent: null },
      content: [
        {
          text: 'Table component',
          type: 'text',
          marks: [
            {
              type: 'insertion',
              attrs: {
                date: '2023-08-04T10:00:00.000Z',
                user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                approved: true,
                username: 'm.sadura@outlook.com'
              }
            }
          ]
        }
      ]
    },
    {
      type: 'table',
      attrs: { sort: { col: null, dir: null }, track: [], labels: [], filters: [], headers: true },
      content: [
        {
          type: 'table_row',
          attrs: { track: [], hidden: false },
          content: [
            {
              type: 'table_header',
              attrs: {
                id: '',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [{ type: 'paragraph', attrs: { track: [] }, content: [{ text: 'Header 1', type: 'text' }] }]
            },
            {
              type: 'table_header',
              attrs: {
                id: '',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [{ type: 'paragraph', attrs: { track: [] }, content: [{ text: 'Header 2', type: 'text' }] }]
            },
            {
              type: 'table_header',
              attrs: {
                id: '',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [{ type: 'paragraph', attrs: { track: [] }, content: [{ text: 'Header 3', type: 'text' }] }]
            }
          ]
        },
        {
          type: 'table_row',
          attrs: { track: [], hidden: false },
          content: [
            {
              type: 'table_cell',
              attrs: {
                id: '_3j6dfs9sx',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [{ type: 'paragraph', attrs: { track: [] }, content: [{ text: 'Cell 1', type: 'text' }] }]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_3j6dfs9sx',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [{ type: 'paragraph', attrs: { track: [] }, content: [{ text: 'Cell 2', type: 'text' }] }]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_3j6dfs9sx',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [{ type: 'paragraph', attrs: { track: [] }, content: [{ text: 'Cell 3', type: 'text' }] }]
            }
          ]
        },
        {
          type: 'table_row',
          attrs: { track: [], hidden: false },
          content: [
            {
              type: 'table_cell',
              attrs: {
                id: '_2lbqldr7l',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: 'row 2',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    },
                    {
                      type: 'tabIndent',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_6rbdb3c86',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '2',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_ue55qho9k',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '2',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'table_row',
          attrs: { track: [], hidden: false },
          content: [
            {
              type: 'table_cell',
              attrs: {
                id: '_6ylp2zqka',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '3',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_c35mtsrgz',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '3',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_9as2scb65',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '3',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          type: 'table_row',
          attrs: { track: [], hidden: false },
          content: [
            {
              type: 'table_cell',
              attrs: {
                id: '_zt6omdot3',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '4',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_bxbqqmpsb',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '4',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            },
            {
              type: 'table_cell',
              attrs: {
                id: '_62627ceze',
                type: 'text',
                track: [],
                header: false,
                colspan: 1,
                rowspan: 1,
                colwidth: null,
                background: 'transparent',
                typesValues: { date: -1, text: '', labels: [], number: '', checkbox: false, currency: '' }
              },
              content: [
                {
                  type: 'paragraph',
                  attrs: { track: [] },
                  content: [
                    {
                      text: '4',
                      type: 'text',
                      marks: [
                        {
                          type: 'insertion',
                          attrs: {
                            date: '2023-08-04T10:00:00.000Z',
                            user: '256638cc-980a-4f03-99be-343bdc1e2a96',
                            approved: true,
                            username: 'm.sadura@outlook.com'
                          }
                        }
                      ]
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    { type: 'paragraph', attrs: { track: [] } }
  ]
};

export const contentWithMedia = {
  type: 'doc',
  content: [
    {
      type: 'image',
      attrs: {
        alt: null,
        src: 'https://static.wixstatic.com/media/1d6dff_c42fbc3f9f4e4f9a9ca10433799a1d3b~mv2.png/v1/fill/w_539,h_514,al_c,lg_1,q_85,enc_auto/1d6dff_c42fbc3f9f4e4f9a9ca10433799a1d3b~mv2.png',
        size: 425,
        track: [],
        caption: null
      }
    },
    { type: 'horizontalRule', attrs: { track: [] } },
    {
      type: 'file',
      attrs: {
        src: 'https://s3.amazonaws.com/charm.public.dev/user-content/256638cc-980a-4f03-99be-343bdc1e2a96/aaa77300-e744-4569-9c24-d9aa795520d0/Status.pdf',
        name: 'Status.pdf',
        size: 143671,
        track: []
      }
    },
    { type: 'horizontalRule', attrs: { track: [] } },
    { type: 'tweet', attrs: { id: '1687388772931907584', track: [], screenName: 'CharmVerse' } },
    { type: 'horizontalRule', attrs: { track: [] } },
    {
      type: 'pdf',
      attrs: {
        src: 'https://s3.amazonaws.com/charm.public.dev/user-content/256638cc-980a-4f03-99be-343bdc1e2a96/44d51e6e-b19a-4143-9ff9-5a34a53a2c24/sample.pdf',
        size: 750,
        track: []
      }
    },
    { type: 'horizontalRule', attrs: { track: [] } },
    { type: 'paragraph', attrs: { track: [] } }
  ]
};
