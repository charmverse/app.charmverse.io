...
import { getUserS3FilePath, uploadToS3 } from 'lib/aws/uploadToS3Server';
...
async function connectDiscord (req: NextApiRequest, res: NextApiResponse<ConnectDiscordResponse | { error: string }>) {
  ...
  if (avatarUrl) {
    try {
      ({ url: avatar } = await uploadToS3({ fileName: getUserS3FilePath({ userId, url: avatarUrl }), url: avatarUrl }));
    }
    catch (err) {
