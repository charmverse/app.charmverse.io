...
import { getBuildersByFid } from '../social/getBuildersByFid';
...
const fids = [
  {
    fid: 547807,
    description:
      'Piesrtasy is the co-founder of DeFi platform HAI and the lead developer at Reflexer Finance, the team behind the RAI stablecoin, which Vitalik has called an "ideal type of a collateralized automated stablecoin”'
  },
  {
    fid: 841580,
    description:
...
export async function getStarterpackBuilders() {
  const farcasterIds = fids.map((fid) => fid.fid);
  const data = await getBuildersByFid({ fids: farcasterIds, season: currentSeason, limit: 100 });

  const builders: BuilderInfo[] = [];

  for (const builder of data.builders) {
    const fid = Number(builder.farcasterId);
    const description = fids.find((f) => f.fid === fid)?.description;
    builders.push({ ...builder, bio: description });
  }
