...
import { LINK as linkPluginName } from '../../nodeNames';
import { mapSlice, matchAllPlus, assertNotUndefined, filter } from '../../utils';
...
// insert links when text is selected
...
// handle pasted text and wrap with link marks
function markPasteRule(regexp: RegExp, type: MarkType, getAttrs: (...params: any[]) => any) {
  return new Plugin({
    props: {
      transformPasted: function transformPasted(slice) {
        return mapSlice(slice, (node) => {
          ...
          return matches.map(({ start, end, match, subString }) => {
            ...
            if (match) {
              const attrs = getAttrs(subString);
...
const LINK_REGEX =
  /(http|https|ftp)?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{2,256}(\.[a-zA-Z]{2,}\b)?([-a-zA-Z0-9@:%_'"+.~#?&//=]*)/g;

export function plugins() {
  return ({ schema }: { schema: Schema }) => {
    const type = schema.marks[linkPluginName];
    return [
