...
export async function importFromWorkspace({
  ...
  const { failedImportsRecord, pagesRecord, pagesWithoutIntegrationAccess } = await notionImporter.import({
    workspaceName,
    workspaceIcon
  });

  const pagesRecordValues = Array.from(pagesRecord.values());
  ...
  log.info('[notion] Completed import of Notion pages', {
    ...
    'Failed import pages': Object.values(failedImportsRecord).slice(0, 25),
    'Pages without integration access': pagesWithoutIntegrationAccess
  });
