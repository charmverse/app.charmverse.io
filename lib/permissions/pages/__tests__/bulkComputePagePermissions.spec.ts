...
describe('bulkComputePagePermissions', () => {
  ...
  it('should always return the same results as computePagePermissions for proposal pages', async () => {
    ...
    const firstSpaceEvaluationActiveProposal = await testUtilsProposals.generateProposal({
      spaceId: firstSpace.id,
      userId: firstSpaceAdmin.id,
    ...
    const secondSpaceVoteActiveProposal = await testUtilsProposals.generateProposal({
      spaceId: secondSpace.id,
      userId: secondSpaceAdmin.id,
    ...
    const spaceWithPublicProposalsVoteActiveProposal = await testUtilsProposals.generateProposal({
      spaceId: spaceWithPublicProposals.id,
      userId: spaceWithPublicProposalsAdmin.id,
      proposalStatus: 'vote_active'
    });
