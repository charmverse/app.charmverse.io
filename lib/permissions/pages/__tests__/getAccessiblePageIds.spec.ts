...
describe('getAccessiblePageIds - public space', () => {
  it('Should return all pages for the space', async () => {
    ...
    const pages = await getAccessiblePageIds({
      userId: undefined,
      spaceId: space.id
    });
    ...
    expect(pages.some((page) => page === page1.id)).toBe(true);
    expect(pages.some((page) => page === page2.id)).toBe(true);
  });
  it('should return only pages marked as deletedAt when user requests archived pages', async () => {
    ...
    const softDeletedPages = await getAccessiblePageIds({
      userId: undefined,
      spaceId: space.id,
      archived: true
    });
    ...
    expect(softDeletedPages[0]).toBe(page2.id);
  });
...
describe('getAccessiblePageIds - public space search', () => {
  it('should return a page based on a title match', async () => {
    ...
    expect(pages[0]).toBe(pageToFind.id);
  });
