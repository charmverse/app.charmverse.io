...
describe('submitEvaluationResult()', () => {
  ...
  it('Should throw error if user has already submitted evaluation result for pass_fail step with > 1 required reviews', async () => {
    const { space, user } = await generateUserAndSpace({
      isAdmin: false
    });
    const { id: proposalId } = await testUtilsProposals.generateProposal({
    ...
    expect(proposalEvaluationReview).toBeTruthy();
    expect(updatedEvaluation.result).toBeNull();
    ...
    ).rejects.toThrow(ActionNotPermittedError);
  });
