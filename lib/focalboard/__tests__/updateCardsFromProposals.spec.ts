...
describe('updateCardsFromProposals()', () => {
  ...
  it('should create cards from proposals if there are new proposals added', async () => {
    ...
    const proposalEvaluationAverageProp = properties.find(
      (prop) => prop.type === 'proposalEvaluationAverage'
    ) as IPropertyTemplate;
    const proposalEvaluationTotalProp = properties.find(
      (prop) => prop.type === 'proposalEvaluationTotal'
    ) as IPropertyTemplate;
    const proposalEvaluatedByProp = properties.find((prop) => prop.type === 'proposalEvaluatedBy') as IPropertyTemplate;
    ...
    expect(card1Properties[proposalEvaluationAverageProp.id]).toStrictEqual([
      {
        title: 'Rubric Evaluation 1',
        value: 5.75
      },
    ...
    expect(card1Properties[proposalEvaluationTotalProp.id]).toStrictEqual([
      {
        title: 'Rubric Evaluation 1',
        value: 5 + 8 + 3 + 7
      },
    ...
    expect(
      (
        card1Properties[proposalEvaluatedByProp.id] as unknown as {
          title: string;
          value: string[];
        }[]
      ).map((prop) => ({
        title: prop.title,
        value: prop.value.sort()
      }))
    ).toStrictEqual([
      {
        title: generatedProposal1.evaluations[0].title,
        value: [proposalReviewer.id, admin.id].sort()
      },
      {
        title: generatedProposal1.evaluations[1].title,
        value: [proposalReviewer.id].sort()
      }
    ]);

    expect(card2Properties[proposalEvaluationAverageProp.id]).toStrictEqual([
      {
        title: 'Rubric Evaluation 1',
        value: 4
      }
    ...
    expect(card2Properties[proposalEvaluationTotalProp.id]).toStrictEqual([
      {
        title: 'Rubric Evaluation 1',
        value: 4
      }
    ...
    expect(
      (
        card2Properties[proposalEvaluatedByProp.id] as unknown as {
          title: string;
          value: string[];
        }[]
      ).map((prop) => ({
      ...
      {
        title: generatedProposal2.evaluations[0].title,
        value: [proposalReviewer.id].sort()
      }
    ]);
