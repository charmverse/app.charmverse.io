...
export async function createWorkspace({
  ...
  if (spaceTemplate === 'default') {
    ...
    const validSpaceTemplate =
      !!staticSpaceTemplates.find((template) => template.id === spaceTemplate) ||
      !!internalTemplates.find((template) => template === spaceTemplate)
        ? spaceTemplate
        : null;

    if (validSpaceTemplate) {
      const resolvedPath = path.resolve(path.join('lib', 'templates', 'exports', `${validSpaceTemplate}.json`));
