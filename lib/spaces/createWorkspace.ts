...
import { SpaceTemplateMapping, spaceContentTemplates } from './config';
...
export async function createWorkspace({
  ...
  if (!createSpaceTemplate || createSpaceTemplate === 'default') {
    ...
    (spaceContentTemplates as Record<string, string>)[SpaceTemplateMapping[createSpaceTemplate as SpaceTemplateType]]
  ) {
    const resolvedPath = path.resolve(
      path.join('lib', 'templates', 'exports', `${SpaceTemplateMapping[createSpaceTemplate as SpaceTemplateType]}.json`)
    );
