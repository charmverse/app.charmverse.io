...
describe('importWorkspacePages', () => {
  ...
  it('should copy proposal evaluations and permissions when coming from a different space', async () => {});
});
describe('importWorkspacePages - proposal content', () => {
    ...
    sourceProposal = await testUtilsProposals
      .generateProposal({
        ...
        evaluationInputs: [
          {
            title: 'Feedback',
            permissions: [{ assignee: { group: 'role', id: sourceSpaceRole.id }, operation: 'comment' }],
  ...
  it('should port over all the proposal configuration when importing within the same space', async () => {
    const data = await exportWorkspacePages({
      sourceSpaceIdOrDomain: sourceSpace.domain,
      // Proposal and page ID can be different in prod, test generator keeps ids the same
      rootPageIds: [sourceProposal.id]
    });
