...
type GenerateImportWorkspacePagesInput = WorkspaceImportOptions & {
  targetSpace: Space;
...
}: GenerateImportWorkspacePagesInput): NewPagesOutput {
    ...
    if (node.type.match('card')) {
      ...
      if (boardBlock && ((!node.type.match('inline') && viewBlocks.length > 0) || node.type.match('inline'))) {
        [boardBlock, ...viewBlocks].forEach((block) => {
          ...
          block.updatedBy = targetSpace.updatedBy;
