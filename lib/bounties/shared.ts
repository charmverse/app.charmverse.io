...
export function requesterCanDeleteBounty({
  bounty,
  requesterIsAdmin,
  requesterCreatedBounty
}: {
  ...
  if (requesterIsAdmin) {
    ...
    requesterCreatedBounty &&
    bounty?.status === 'open' &&
    bounty?.applications.filter((a) => a.status !== 'applied' && a.status !== 'rejected').length === 0
  ) {
    return true;
  }
...
export function getBountyPagePermissionSet(
  model: BountyPagePermissionSetRequest
): Omit<Prisma.PagePermissionCreateManyInput, 'pageId'>[] {
