...
describe('listAvailableBounties', () => {
  it('should return all bounties in the space', async () => {
    ...
    const availableForAdmin = await listAvailableBounties({
      spaceId: space.id,
      userId: adminUser.id
    });

    const availableForMember = await listAvailableBounties({
      spaceId: space.id,
      userId: nonAdminUser.id
    });

    const availableForPublic = await listAvailableBounties({
      spaceId: space.id,
      userId: undefined
    });
  ...
  it('should exclude deleted bounties from the results', async () => {
    ...
    const result = await listAvailableBounties({
      spaceId: space.id,
      userId: adminUser.id
    });
