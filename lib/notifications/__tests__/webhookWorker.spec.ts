...
import { handleWebhookEvent } from 'serverless/webhook/worker';
import { v4 } from 'uuid';
...
describe(`Test card events and notifications`, () => {
  it(`Should create card notifications for mention.created event`, async () => {
    const { space, user } = await generateUserAndSpaceWithApiToken();
    const user2 = await createUserFromWallet();
    await addUserToSpace({
      spaceId: space.id,
    ...
    const cardNotification = await prisma.cardNotification.findFirst({
      where: {
        type: 'mention.created',
        mentionId,
  ...
  it(`Should create card notifications for inline_comment.created event`, async () => {
    const { space, user } = await generateUserAndSpaceWithApiToken();
    const user2 = await createUserFromWallet();
    await addUserToSpace({
      spaceId: space.id,
    ...
    await handleWebhookEvent({
      event: {
        ...
        card: await getCardEntity(cardPage.page.id),
        space: await getSpaceEntity(space.id),
        inlineComment: await getInlineCommentEntity(inlineComment.id)
      },
      id: v4(),
    ...
    const inlineCommentCreatedNotification = await prisma.cardNotification.findFirst({
      where: {
        type: 'inline_comment.created',
        inlineCommentId: inlineComment.id,
        cardId: cardPage.block.id,
    ...
    const inlineCommentRepliedNotification = await prisma.cardNotification.findFirst({
      where: {
        type: 'inline_comment.replied',
        inlineCommentId: inlineComment.id,
        cardId: cardPage.block.id,
    ...
    const inlineCommentMentionCreatedNotification = await prisma.cardNotification.findFirst({
      where: {
        type: 'inline_comment.mention.created',
        inlineCommentId: inlineComment.id,
        mentionId,
  ...
  it.only(`Should create card notifications for block_comment.created event`, async () => {
    const { space, user } = await generateUserAndSpaceWithApiToken();
    const user2 = await createUserFromWallet();
    await addUserToSpace({
      spaceId: space.id,
