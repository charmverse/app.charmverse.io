    ...
    const reduced = mapped.reduce(
      (reviewersByGroup, reviewer) => {
        if (reviewer.group === 'role') {
          ...
          const usersToAdd = (roleups.find((r) => r.id === roleAsReviewer.id)?.users ?? []).map((u) => {
            return {
              id: u.id,
              name: u.username,
              profilePic: u.avatar,
      ...
      {reviewerNames.users.length > 0 && (
        <Box display='flex' alignItems='center'>
          <AvatarGroup
            max={maxVisibleUsers}
            sx={{ mr: 1 }}
            onClick={() => setMaxVisibleUsers(maxVisibleUsers + defaultAvatarGroupIncrement)}
          >
