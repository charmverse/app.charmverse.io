...
function MultipleAddCondition({
  ...
  const { setDisplayedPage } = useContext(ShareModalContext);
    ...
    if (selectPage === 'chooseAccess') {
      ...
      if (isNested === true) {
        ...
        Object.keys(chain.types.conditionTypes).forEach((c, i) => {
          if (
            (!conditionTypeData[c].supportedChains || conditionTypeData[c].supportedChains.includes(chain.value)) &&
            (!chain.disallowNesting || !chain.disallowNesting.find((n) => n === c))
          ) {
            allowedNestedConditions.push(
              <LitChooseAccessButton
                key={i}
                onClick={() => setSelectPage(c)}
      ...
      return (
        ...
        <div className='lsm-multiple-condition-rendering-options-container'>
          <h3 className='lsm-multiple-condition-select-prompt'>Choose who can access this:</h3>
      ...
      if (chain.types.conditionTypes[selectPage]) {
        ...
        return (
          <ConditionHolder
            updateUnifiedAccessControlConditions={setUnifiedAccessControlConditions}
            chain={chain}
  ...
  return (
    <div className='lsm-multiple-condition-add-container'>
      <LitChainSelector />
      {!!chain && !!setSelectPage && (
        <div className='lsm-multiple-select-condition-display'>{getRenderedConditionOption()}</div>
      )}
