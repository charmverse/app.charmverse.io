...
/**
 * remove todoChecked attribute from a list_item
 * no-op if not list_item
 * @param {*} tr
 * @param {*} schema
 * @param {*} node
 * @param {*} pos
...
/**
 * set todoChecked attribute to a list_item
 * no-op if not list_item or if already todo
 * @param {*} tr
 * @param {*} schema
 * @param {*} node
 * @param {*} pos
...
export const setTodoCheckedAttr = (tr: Transaction, schema: Schema, node: Node, pos: number) => {
  if (node.type === schema.nodes.list_item && !isNodeTodo(node, schema)) {
...
export function wrappingInputRuleForTodo(
  ...
  return new InputRule(regexp, (state, match, start, end) => {
    ...
    if (
      before &&
      before.type === state.schema.nodes.bullet_list &&
...
function isSelectionParentBulletList(state: EditorState) {
  ...
  return (
    fromNode &&
    fromNode.type === state.schema.nodes.bullet_list &&
    endNode &&
    endNode.type === state.schema.nodes.bullet_list
  );
}
...
function todoCount(state: EditorState) {
  ...
  siblingsAndNodesBetween(state, (node) => {
    // TODO it might create problem by counting ol 's list_item?
