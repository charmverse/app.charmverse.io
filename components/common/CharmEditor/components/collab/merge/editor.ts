...
export class MergeEditor {
        ...
        const helpDialog = new FaqDialog({
          ...
          questions: [
            ...
            [
              ...
              { mergeImage: `<img src="${globalThis.settings_STATIC_URL}img/accept-change.png" class = "merge-img">` },
              { hasImage: true }
            ],
            [
  ...
  checkResolution () {
    ...
    const mergedVersionDoc = this.mergeView2.state.doc;
    ...
    tr.steps.forEach((step, index) => {
      ...
      if (step instanceof ReplaceStep && !stepsTrackedByChangeset.includes(index)) {
        ...
        if (stepOne.slice && stepOne.slice.content.length === 1 && stepOne.slice.content[0].type === 'footnote') {
          const insertionMark = this.schema.marks.diffdata.create({ diff: insertionClass,
  ...
  startMerge (offlineTr: Transaction, _onlineDoc) {
    try {
      ...
      this.applyChangesToMainEditor(_onlineDoc, mergedDoc);
  ...
  unHideSections (view: EditorView) {
    let offset = 1; let
      attrs;
    ...
    tr.doc.nodesBetween(
      ...
      (node, pos) => {
        ...
        if (node.attrs.diffdata) {
          const diffdata = [];
          diffdata.push({ type: difftype, from, to, steps: stepsInvolved });
          tr.setNodeMarkup(pos, null, { ...node.attrs, diffdata }, node.marks);
        }
      }
    );
  }
  ...
  updateDB (offlineDoc, db) {
    ...
    offlineDoc.descendants(node => {
      if (node.type.name === 'image' && node.attrs.image) {
      ...
      if (!this.editor.mod.db.imageDB.db[id] && oldImageDB[id]) {
        ...
        if (Object.keys(this.editor.app.imageDB.db).includes(
          ...
          this.editor.mod.db.imageDB.reUploadImage(id, oldImageDB[id].image, oldImageDB[id].title, oldImageDB[id].copyright).then(
            ({ id: _id, newId }) => {
              this.imageDataModified[_id] = newId;
              // Update the image node if there are any re uploaded images.
              this.mergeView1.state.doc.descendants((node: Node, pos: number) => {
                if (node.type.name === 'image' && node.attrs.image === _id) {
              ...
              this.mergeView3.state.doc.descendants((node: Node, pos: number) => {
                if (node.type.name === 'image' && node.attrs.image === _id) {
                  const attrs = { ...node.attrs };
                  attrs.image = newId;
