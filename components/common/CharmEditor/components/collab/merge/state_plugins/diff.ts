...
import {
  ...
  Mapping,
  DecorationSet,
  Decoration } from '@bangle.dev/pm';

import { noSpaceTmp } from '../../../../common';
import { ChangeSet } from '../Ct';
import {
...
function createHiglightDecoration (from: number, to: number, state: EditorState) {
...
function getDecos (decos, merge, state) {
  ...
  const currentMarks = [];
  const diffMark = $head.marks().find(
  ...
  if (!currentMarks.length) {
    ...
    if (node && node.attrs.diffdata && node.attrs.diffdata.length > 0) {
      ...
      const deco = Decoration.widget(startPos, dom);
    ...
    else if (node.marks.find(m => m.type.name === 'diffdata')) {
      const mark = node.marks.find(m => m.type.name === 'diffdata');
      const startPos = $head.pos;// position of block start.
...
function deletionDecorations (decos: DecorationSet, merge, doc: Node, tr: Transaction, deletionClass: string) {
  let index = 0;
  let stepsTrackedByChangeset: any[] = [];
  const changeset = new ChangeSet(tr).getChangeSet();
  const schema = merge.schema;
  ...
  changeset.changes.forEach(change => {
    if (change.deleted.length > 0) {
      ...
      const pos = mapping.map(change.fromA);
      decos = decos.add(doc, [
...
function createDropUp (merge, diffMark, linkMark) {
  ...
  const requiredPx = 10;
  const tr = diffMark.attrs.diff.search('offline') !== -1 ? merge.offlineTr : merge.onlineTr;
  const trType = diffMark.attrs.diff.search('offline') !== -1 ? 'offline' : 'online';
  const opType = diffMark.attrs.diff.search('inserted') !== -1 ? 'insertion' : 'deletion';
  let textToBeDisplayed = '';

  if (diffMark.attrs.markOnly) {
    textToBeDisplayed = gettext('Format Change');
  }
  else if (trType === 'online') {
    if (opType === 'insertion') {
      ...
      (event) => {
        ...
        try {
          if (trType === 'online') {
            ...
            // remove online deletion decoration
            else if (addDeletedContentBack(merge, merge.mergeView2, diffMark)) {
              const decorationId = dropUp.dataset.decoid;
              removeDecoration(merge.mergeView2, decorationId);
            }
          }
