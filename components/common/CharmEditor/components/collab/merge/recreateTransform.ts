...
import { Transform, ReplaceStep } from '@bangle.dev/pm';
import { diffWordsWithSpace, diffChars } from 'diff';
import { applyPatch, createPatch } from 'rfc6902';

function getReplaceStep (fromDoc: Node, toDoc: Node) {
  let start = toDoc.content.findDiffStart(fromDoc.content) as number | null;
  if (start === null) {
    return false;
  }
  let {
...
class RecreateTransform {
  ...
  constructor (fromDoc: Node, toDoc: Node, complexSteps, wordDiffs) {
