...
export function autoLinkInputRule(type: MarkType) {
...
export function pasteLink(regexp: RegExp) {
  return new Plugin({
    props: {
...
export function markPasteRule(regexp: RegExp, type: MarkType, getAttrs: (...params: any[]) => any) {
  return new Plugin({
    props: {
...
const LINK_REGEX = /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{2,256}\.[a-zA-Z]{2,}\b([-a-zA-Z0-9@:%_'"+.~#?&//=]*)/g;
