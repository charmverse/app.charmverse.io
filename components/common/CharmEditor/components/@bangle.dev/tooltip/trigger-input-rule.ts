...
import { InputRule, TextSelection } from '@bangle.dev/pm';
...
export function triggerInputRule(schema: Schema, markName: string, trigger: string) {
  ...
  const startRule = new InputRule(regexStart, (editorState: EditorState, match: string[]) => {
     ...
     */
    const _trigger = match[3] || match[2];
    if (!_trigger) {
      return null;
    }

    const mark = schema.mark(markName, { trigger: _trigger });
    ...
    if (_trigger.length > 1) {
      const textSelection = TextSelection.create(tr.doc, selection.from, selection.from - _trigger.length + 1);
      tr.setSelection(textSelection);
    }
    const marks = selection.$from.marks(); // selection would tell the cursor position, in this case from == to as no selection
    return tr.replaceSelectionWith(schema.text(_trigger, [mark, ...marks]), false);
  });
