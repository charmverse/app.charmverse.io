...
import type { AssignedRewardPermissions, RewardWithDetails } from 'lib/bounties';
import { isRewardLockable } from 'lib/bounties/shared';

import RewardApplicantTableRow from './RewardApplicantTableRow';

interface Props {
  bounty: RewardWithDetails;
  permissions: AssignedRewardPermissions;
}

export default function RewardSubmissionsTable({ bounty, permissions }: Props) {
  ...
  const { refreshReward } = useBounties();
  const { user } = useUser();
  ...
  async function lockRewardSubmissions() {
    const updatedReward = await charmClient.bounties.lockSubmissions(bounty!.id, !bounty.submissionsLocked);
    ...
    <>
      <Box width='100%' display='flex' mb={1} justifyContent='space-between'>
        <Box display='flex' gap={1} alignItems='center'>
          ...
          />
          {permissions?.userPermissions?.lock && isRewardLockable(bounty) && (
            <Tooltip
              key='stop-new'
              arrow
              placement='top'
