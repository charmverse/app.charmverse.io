...
function markDeletion(
  ...
  tr.doc.nodesBetween(from, to, (node, pos, _parent, _index) => {
    if (pos < from && node.type.name === 'table_cell') {
      ...
      node.isInline &&
      node.marks.find((mark) => mark.type.name === 'insertion' && mark.attrs.user === user.id && !mark.attrs.approved)
    ) {
...
export function trackedTransaction(
  ...
  tr.steps.forEach((originalStep, originalStepIndex) => {
    ...
    if (step instanceof ReplaceStep) {
      ...
      } else if (!step.slice.size || (step.slice.content as any).content.length === 2) {
      ...
      doc.nodesBetween(step.from, step.to, (node, pos) => {
        ...
        if (SUPPORTED_MARKS.includes(step.mark.type.name) && !node.marks.find((mark) => mark.type === step.mark.type)) {
