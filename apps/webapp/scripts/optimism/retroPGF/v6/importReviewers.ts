...
const reviewerData = getCsvData<CSVRow>('../__data/optimism/reviewer_allocations.tsv');
...
function getCsvData<T>(path: string): T[] {
  return parse(readFileSync(path).toString(), { columns: true, delimiter: '\t' }) as T[];
}

async function populateProject(csvRow: CSVRow, proposalPages: DatabaseProposals, emailToUser: Record<string, string>) {
  const page = proposalPages.find((r) => r.title.trim() === csvRow['Project Name'].trim());
  if (!page) {
    throw new Error('Application not found for ' + csvRow['Application ID']);
  }
  ...
  const reviewers = [
    ...
    csvRow['Reviewer 4'],
    csvRow['Reviewer 5']
  ].map((email) => emailToUser[email.toLowerCase().trim()]);

  const appealReviewers: any[] = [];
...
async function importReviewers() {
  ...
  for (const project of populatedProjects) {
    ...
    await prisma.$transaction([
      // uncomment the following two updates to complete the first spam check
...
// createUsers([
//   'op_profile_292s7fdh@cheeky-gorilla.lol',
//   'waterproofeth@gmail.com',
//   'jakub.mucha@icloud.com',
//   'okikifamutimi@gmail.com',
//   'matt@ethereum.org',
//   'webby1111@hotmail.com'
// ]).catch((e) => {
//   console.error('Error crashed script', e);
//   process.exit(1);
// });
