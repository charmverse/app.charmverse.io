...
// sessionStorage hook inspiration: https://blog.logrocket.com/using-localStorage-react-hooks/

export function getStorageValue<T = any>(key: string, defaultValue: T, noPrefix?: boolean): T {
  if (typeof window !== 'undefined') {
    const saved = sessionStorage.getItem(getKey(key, noPrefix));
...
export function setStorageValue<T = any>(key: string, value: T, noPrefix?: boolean): T {
  sessionStorage.setItem(getKey(key, noPrefix), JSON.stringify(value));
  return value;
}

// value is null until we have a chance to check local storage
export function useSessionStorage<T = any>(key: string | null, defaultValue: T, noPrefix?: boolean) {
